...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!clearBtn || !huoltoLista) return;

    clearBtn.addEventListener('click', () => {
        const ok = confirm(
            'Haluatko varmasti poistaa kaikki huoltomerkinnät? Tätä toimintoa ei voi perua.'
        );
        if (!ok) return;

        huoltoLista.innerHTML = '';

        localStorage.removeItem('huollot');

        alert('Kaikki merkinnät poistettiin.');
    });
});